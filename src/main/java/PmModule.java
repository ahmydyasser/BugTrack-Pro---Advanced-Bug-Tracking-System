...
public class PmModule {
    ...
    public Set<String> getAllProjects() {
        ...
        for (Bug bug : allBugs) {
            if (bug.projectName != null) projects.add(bug.projectName);
        }
        return projects;
    }
    ...
    public List<Bug> filterBugs(String project, String status) {
        ...
        for (Bug bug : allBugs) {
            boolean matchesProject = project.equals("All") || (bug.projectName != null && bug.projectName.equals(project));
    ...
    public Map<String, Integer> getDevBugStats() {
        ...
        for (Bug bug : allBugs) {
            if (bug.assignedDev != null && !bug.assignedDev.isEmpty()) {
                devBugs.put(bug.assignedDev, devBugs.getOrDefault(bug.assignedDev, 0) + 1);
            }
        }
        return devBugs;
    }
    ...
    public int getOpenCount() {
        int open = 0;
        for (Bug bug : allBugs) {
            if ("closed".equals(bug.status)) continue;
            open++;
        }
        return open;
    }
    ...
    public int getClosedCount() {
        int closed = 0;
        for (Bug bug : allBugs) {
            if ("closed".equals(bug.status)) closed++;
        }
        return closed;
    }
} 